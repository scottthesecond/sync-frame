...
export class AirtableAdapter implements SourceAdapter {
  ...
  async getUpdates(
    ...
    return {
      changes: {
        upserts: allRecords,
        deletes: [], // Airtable doesn't expose deleted records in the API
        // Deletions would need to be tracked via a soft-delete flag field or
        // detected through other means (e.g., comparing with previous sync state)
      },
      nextCursor,
    };
  }
